...
import {
  FaXmark, FaBars, FaHouseChimney, FaClipboardList,
  FaClock, FaFileLines, FaFolderOpen, FaCalendarDays,
  ...
  return (
    <aside className="w-64 bg-[#18AD8F] text-white flex flex-col">
      ...
      <div className="p-6 border-b border-white/20">
        ...
        <FaBars size={22} />
        <img src={SFlogo} alt="SociaFast" className="w-40 mx-auto" />
      </div>

      <nav className="flex flex-col gap-2 p-4">
        <NavLink to="/dashboardAlumno" className={({ isActive }) =>
        ...
        <Link to="/historial" className="flex items-center gap-3 w-full p-3 rounded-lg hover:bg-white/20 transition">
          <FaClipboardList size={22} /><span>Historial</span>
        </Link>

        <Link to="/extracurriculares" className="flex items-center gap-3 w-full p-3 rounded-lg hover:bg-white/20 transition">
      ...
      <div className="mt-auto p-4 border-t border-white/20">
        ...
        {/* onLogout viene del Dashboard — limpia sesión y redirige */}
        <button
          onClick={onLogout}
          className="flex items-center gap-2 w-full bg-white/10 rounded-lg p-4 hover:bg-red-500/60 transition"
        >
